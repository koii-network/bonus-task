...
export async function task(roundNumber) {
  ...
  try {
    // Check current slot
    const taskState = await namespaceWrapper.getTaskState({}); // Get the task state
    ...
    if ((roundBeginSlot + 120) <= currentSlot) {
      console.log("Task is not ready to be executed yet");
    ...
    await namespaceWrapper.storeSet(
      ...
      console.log("Task missed the window to be executed, skip round", roundNumber);
    }
  } catch (error) {
