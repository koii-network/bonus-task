...
async function harmonizeDistribution ( distribution_proposal ) {
  ...
  let harmonizedDistribution = [];
  let walletKeys = [];
  for (let item of distribution_proposal) {
    let walletKey = item.developer_key || item.node_key;
    if (!walletKeys.includes(walletKey)) {
      walletKeys.push(walletKey);
      harmonizedDistribution.push(item);
    } else {
      let existingItem = harmonizedDistribution.find(i => i.developer_key === walletKey || i.node_key === walletKey);
      if (item.developer_key) {
        existingItem.dev_bonus += item.dev_bonus;
      } else {
        existingItem.node_bonus += item.node_bonus;
      }
    }
  }
  return harmonizedDistribution;
